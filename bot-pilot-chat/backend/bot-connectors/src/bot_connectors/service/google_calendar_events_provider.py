...
from bot_connectors.service.google_calendar_client import GoogleCalendarClient, \
...
class GoogleCalendarEventsProvider(CalendarEventsReader):
    ...
    def read_busy_events_next(self, customer_context: str,
        ...
        try:
            calendar_service = (self._calendar_client
            .get_google_calendar_as_service(
                customer_context))
            if calendar_service is None:
                return None
            now = datetime.utcnow().isoformat() + "Z"
            time_max = (datetime.utcnow() + timedelta(
                days=next_days)).isoformat() + 'Z'
            body = {
                'timeMin': now,
                'timeMax': time_max,
                'timeZone': 'UTC',
            ...
            logger.error(f'Failed to get busy events for [{customer_context}]:'
                         f'[{e}]')
            raise


def get_google_calendar_events_provider(
        calendar_client: GoogleCalendarClient = Depends(
            get_google_calendar_client)):
